...
"""Borrowed from https://github.com/hoel-bagard/torch_utils/blob/master/utils/prepare_folders.py"""
...
def prepare_folders(checkpoints_dir: Optional[Path] = None,
                    repo_name: str = "train_code",
                    extra_files: Optional[list[Path]] = None):
    """Prepare  checkpoints folders.
    ...
    if checkpoints_dir is not None:
        ...
        for i, file in enumerate(files_to_copy):
            if "template" in str(file):
                files_to_copy[i] = Path(str(file).replace("_template", ""))
